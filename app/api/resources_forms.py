...
from app.api.resources_schemas import (
    ErrorResponse, FormListResponse, FormPath, FormVersionPath, ListQuery, MessageResponse,
    ProjectPath, WorkflowActionRequest, WorkflowActionResponse,
)
...
def list_forms(path: ProjectPath, query: ListQuery):
    ...
    return to_json_ready(FormListResponse(
...
def get_effective_ui_config(path: FormPath):
    ...
    effective = _deep_merge({"theme": theme_config, "layout": layout_config}, ui_overrides)
    return to_json_ready(EffectiveUiConfigOutput(
...
def _workflow_action(path: FormPath, body: WorkflowActionRequest, action: str):
    ...
    try:
        message = _apply_form_workflow_action(
            form=form, action=action, actor_user_uuid=actor_user_uuid, note=body.note,
        )
    except ValueError as exc:
        _security_event(
    ...
    return to_json_ready(WorkflowActionResponse(
        message=message, action=action,
        actor_user_uuid=actor_user_uuid, form_uuid=form.uuid, project_uuid=project.uuid,
    ))
