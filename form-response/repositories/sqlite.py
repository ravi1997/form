...
class SQLiteRepository(RepositoryInterface):
    ...
    def get_form(self, form_id: str) -> FormSnapshot | None:
        with self._connect() as conn:
            row = conn.execute("SELECT payload FROM forms WHERE form_id = ?", (form_id,)).fetchone()
        if not row:
            return None
    ...
    def upsert_form(self, form: FormSnapshot) -> None:
        ...
        with self._connect() as conn:
            conn.execute(
                """
                INSERT INTO forms(form_id, payload, created_at, updated_at)
                VALUES(?, ?, ?, ?)
                ON CONFLICT(form_id) DO UPDATE SET
                    payload = excluded.payload,
                    updated_at = excluded.updated_at
                """,
                (form.form_id, payload, form.created_at, form.updated_at),
            )
